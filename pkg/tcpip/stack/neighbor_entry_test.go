...
func entryTestSetup(c NUDConfigurations) (*neighborEntry, *entryTestDispatcher, *entryTestLinkResolver) {
	disp := entryTestDispatcher{
...
func TestEntryUnknownToUnknownWhenConfirmationWithUnknownAddress(t *testing.T) {
	...
	e.mu.Lock()
	e.handleConfirmationLocked(entryTestLinkAddr1, false, false, false)
	if got, want := e.mu.neigh.State, Unknown; got != want {
		t.Errorf("e.mu.neigh.State=%q, want=%q", got, want)
	}
	e.mu.Unlock()
...
func TestEntryIncompleteToReachableWithRouterFlag(t *testing.T) {
	...
	e.handleConfirmationLocked(entryTestLinkAddr1, true /* solicited */, false /* override */, true /* isRouter */)
	if got, want := e.mu.neigh.State, Reachable; got != want {
		t.Errorf("e.mu.neigh.State=%q, want=%q", got, want)
	}
