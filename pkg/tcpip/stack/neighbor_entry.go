...
// dispatchEventAdded sends an event to notify listening users that a new entry
// has been added, meaning it is now accessible through public APIs.
func (e *neighborEntry) dispatchEventAdded(nextState NeighborState) {
	if e.nic.stack.nudDisp == nil {
		return
	}
	e.nic.stack.nudDisp.OnNeighborAdded(e.nic.id, e.mu.neigh.Addr, e.mu.neigh.LinkAddr, nextState)
}

// dispatchEventStatChange sends an event to notify listening users that as
// entry has changed its state in the NUD state machine as defined by
// RFC 4861 Section 7.3.2.
func (e *neighborEntry) dispatchEventStateChange(nextState NeighborState) {
	if e.nic.stack.nudDisp == nil {
		return
	}
	e.nic.stack.nudDisp.OnNeighborStateChange(e.nic.id, e.mu.neigh.Addr, e.mu.neigh.LinkAddr, nextState)
}

// dispatchEventRemoved sends an event to notify listening users that an entry
// has been effectively removed from the neighbor table, meaning it is no
...
func (e *neighborEntry) dispatchEventRemoved() {
	if e.nic.stack.nudDisp == nil {
		return
	}
	e.nic.stack.nudDisp.OnNeighborRemoved(e.nic.id, e.mu.neigh.Addr, e.mu.neigh.LinkAddr, e.mu.neigh.State)
}
