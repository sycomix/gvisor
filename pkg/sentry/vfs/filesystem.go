...
type FilesystemImpl interface {
	...
	// explanation.)
	//
	// - If the list returned would be larger than size, ERANGE is returned the
	// buffer passed into listxattr(2) is too small (note that if size is 0, the
	// entire list should be returned regardless of size).
	...
	// returned.
	//
	// - If the size of the return value is greater than opts.Size, ERANGE is
	// returned (note that if size is 0, the value should be returned without
