...
func (e *endpoint) handleICMP(r *stack.Route, netHeader buffer.View, pkt stack.PacketBuffer) {
	...
	case header.ICMPv6NeighborSolicit:
		...
		// The remainder of pkt.Data must be only the neighbor
		// solicitation, so pkt.Data.ToView() always returns the
		// solicitation. Per RFC 6980 section 5, NDP messages cannot be
		// fragmented. Also note that in the common case NDP datagrams
		// are very small and ToView() will not incur allocations.
		ns := header.NDPNeighborSolicit(header.ICMPv6(pkt.Data.ToView()).NDPPayload())
	...
	case header.ICMPv6NeighborAdvert:
		...
		if pkt.Data.Size() < header.ICMPv6NeighborAdvertSize {
			received.Invalid.Increment()
			return
		}

		// The remainder of pkt.Data must be only the neighbor
		// advertisement, so pkt.Data.ToView() always returns the
		// advertisement. Per RFC 6980 section 5, NDP messages cannot
		// be fragmented. Also note that in the common case NDP
		// datagrams are very small and ToView() will not incur
		// allocations.
		na := header.NDPNeighborAdvert(header.ICMPv6(pkt.Data.ToView()).NDPPayload())
	...
	case header.ICMPv6RouterAdvert:
		...
		// Advertisement?
		if pkt.Data.Size()-header.ICMPv6HeaderSize < header.NDPRAMinimumSize {
			// ...No, silently drop the packet.
			received.Invalid.Increment()
			return
		}

		// The remainder of pkt.Data must be only the router
		// advertisement, so pkt.Data.ToView() always returns the
		// advertisement. Per RFC 6980 section 5, NDP messages cannot
		// be fragmented. Also note that in the common case NDP
		// datagrams are very small and ToView() will not incur
		// allocations.
		ra := header.NDPRouterAdvert(header.ICMPv6(pkt.Data.ToView()).NDPPayload())
		opts := ra.Options()
