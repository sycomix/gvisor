...
// Recently Used (LRU) eviction strategy to implement a bounded cache. It
// contains the state machine and configuration for running Neighbor
...
func (n *neighborCache) getOrCreateEntry(remoteAddr, localAddr tcpip.Address, linkRes LinkAddressResolver) *neighborEntry {
	...
	if len(n.mu.cache) == neighborCacheSize {
		e := n.mu.lru.Back()
		e.mu.Lock()
...
func (n *neighborCache) addStaticEntry(addr tcpip.Address, linkAddr tcpip.LinkAddress) {
	...
	entry.dispatchEventAdded(Static)
}
...
func (n *neighborCache) removeEntry(addr tcpip.Address) bool {
	...
	entry.mu.Lock()
	entry.dispatchEventRemoved()
	entry.setStateLocked(Failed)
	entry.mu.Unlock()
...
func (n *neighborCache) HandleProbe(remoteAddr, localAddr tcpip.Address, protocol tcpip.NetworkProtocolNumber, remoteLinkAddr tcpip.LinkAddress) {
	if header.IsV4MulticastAddress(remoteAddr) || !header.IsV6UnicastAddress(remoteAddr) {
		return
	}
...
func (n *neighborCache) HandleConfirmation(addr tcpip.Address, linkAddr tcpip.LinkAddress, solicited, override, isRouter bool) {
	n.mu.RLock()
	entry, ok := n.mu.cache[addr]
	n.mu.RUnlock()
	if ok {
		entry.mu.Lock()
		entry.handleConfirmationLocked(linkAddr, solicited, override, isRouter)
		entry.mu.Unlock()
	}
