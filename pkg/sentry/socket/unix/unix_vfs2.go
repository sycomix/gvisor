...
// VFSFD retrieves the vfs.FileDescription contained in s.
func (s *SocketVFS2) VFSFD() *vfs.FileDescription {
	return &s.vfsfd
}
	...
	return s.ep.Bind(tcpip.FullAddress{Addr: tcpip.Address(p)}, func() *syserr.Error {
		...
		if p[0] == 0 {
			...
			err := t.Kernel().VFS().MknodAt(t, t.Credentials(), &pop, &vfs.MknodOptions{
				// TODO(gvisor.dev/issue/1476): Pretty sure the file permissions
				// should be taken from s (see net/unix/af_unix.c:unix_bind), but VFS1
				// just always uses 0400. Resolve this inconsistency.
