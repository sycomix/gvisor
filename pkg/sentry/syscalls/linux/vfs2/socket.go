...
func Socket(t *kernel.Task, args arch.SyscallArguments) (uintptr, *kernel.SyscallControl, error) {
	...
	// Set all I/O to non-blocking so that we can handle blocking behavior within
	// the sentry instead of depending on the host.
	if err := s.SetStatusFlags(t, t.Credentials(), linux.SOCK_NONBLOCK); err != nil {
		return 0, nil, err
	}
...
func SocketPair(t *kernel.Task, args arch.SyscallArguments) (uintptr, *kernel.SyscallControl, error) {
	...
	// Set all I/O to non-blocking so that we can handle blocking behavior within
	// the sentry instead of depending on the host.
	if err := s1.SetStatusFlags(t, t.Credentials(), linux.SOCK_NONBLOCK); err != nil {
		return 0, nil, err
	}
	if err := s2.SetStatusFlags(t, t.Credentials(), linux.SOCK_NONBLOCK); err != nil {
		return 0, nil, err
	}
	...
	fd1, err := t.NewFDFromVFS2(0, s1, flags)
	if err != nil {
		return 0, nil, err
	}
	fd2, err := t.NewFDFromVFS2(0, s2, flags)
	if err != nil {
		return 0, nil, err
	}

	// Copy the file descriptors out.
	if _, err := t.CopyOut(socks, []int32{fd1, fd2}); err != nil {
